    ...
    def search
        ...
        if params[:search].present?
          query = "title ILIKE :search OR organizations.name ILIKE :search 
           ...
           @campaigns = Campaign.active.real.where("id>0").joins(:organization).where(query, search: "%"+@search+"%").order("title, organizations.name, campaigns.id desc").page(params[:page])
        else
