    ...
    def search
        ...
        if params[:search].present?
          query = "title ILIKE :search OR organizations.name ILIKE :search 
           ...
           @campaigns = Campaign.isdestroy.active.joins(:organization).where(query, search: "%"+@search+"%").order("title, organizations.name, campaigns.id desc").page(params[:page])
        else
          ...
          if !@register
            @is_recent = true
            @campaigns = Campaign.isdestroy.active.order(:id=>:desc).page(1)
          end
        end
