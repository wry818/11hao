class Admin::ProductsController < Admin::ApplicationController
  ...
  def index
    ...
    @products = Product.isnot_destroy.order(:updated_at => :desc).page(params[:page])
  end

  def show_pager_data
    @products = Product.where(:product_category_id => params[:product_category_id],
                              :pro_cat_subclass_id => params[:pro_cat_subclass_id]
    ).includes(:product_category, :collections).isnot_destroy.order(:updated_at => :desc).page(params[:page])

    if !params[:product_category_id]||params[:product_category_id].to_i<0
      @products.unscope(where: :product_category_id)
    end
    if !params[:pro_cat_subclass_id]||params[:pro_cat_subclass_id].to_i<0
      @products.unscope(where: :pro_cat_subclass_id)
    end
  ...
  def product_params
    params.require(:product).permit :is_hot_sale,:name, :description, :picture, :base_price, :default_donation_amount, :show_quantity, :is_featured, :fulfillment_method, :sku, :vendor_id, :original_price, :product_category_id, :pro_cat_subclass_id
  end
